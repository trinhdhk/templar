...
prep_orig_text <- function(orig_text, global_eval = TRUE) {
  ...
  start_call <- orig_text %>% stringr::str_which("(?<!_)versions\\(")
  ...
  # Remove the knit_version call in yaml matter
  end_yaml <- stringr::str_which(orig_text, "---")[2] - 1
  knit_call <- stringr::str_which(orig_text[1:end_yaml], "^knit:\\s+(templar\\:\\:)?knit_versions\\(")
  merge_yaml <- identical(orig_text[(knit_call+1):(knit_call+2)], c("---", "---"))

  orig_text <- orig_text[c(-knit_call,
                            if (merge_yaml) c(-knit_call-1, -knit_call-2))]
